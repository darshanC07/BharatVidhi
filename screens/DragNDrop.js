...
const DraggableBlock = ({
  ...
  // In the panResponder of the DraggableBlock component
  const panResponder = React.useMemo(() => {
    ...
    return PanResponder.create({
      ...
      onPanResponderRelease: (e, gestureState) => {
        ...
        // Calculate the distance moved - ONLY HORIZONTAL MOVEMENT
        const distanceMoved = Math.abs(gestureState.dx);

        // Pass the distance moved to onDragEnd
        onDragEnd(id, currentPosition, slotIndex, distanceMoved);

        // Always reset pan to {0, 0} after drag ends to snap back to original position
        pan.setValue({ x: 0, y: 0 });
      },
    });
...
const DropArea = ({ droppedItems, setDroppedItems, onRearrange, onRemoveItem, blocks, setBlocks }) => {
  const [fontsLoaded] = useFonts({
    Itim_400Regular,
  });

  // Create fixed positions for 5 slots with reduced vertical spacing
  const slotPositions = [
    { x: 20, y: 50 },
    { x: 20, y: 120 },
    { x: 20, y: 190 },
  ...
  const handleDragEnd = (id, position, slotIndex, distanceMoved) => {
    onRearrange(id, position, slotIndex, distanceMoved);
  };
  ...
  return (
    ...
    >
      ...
      >
        <Text style={{alignSelf:'center',color:'white',marginTop:5,fontFamily: "Itim_400Regular",fontSize:18,
}}>Reset</Text>
      </TouchableOpacity>
      ...
      {droppedItems.length === 0 ? (
        ...
        <View
          style={{
            ...
            // alignItems: "center",
            // alignSelf: "center",
            // backgroundColor: "rgba(48, 14, 238, 0.15)",
          }}
        >
        ...
        {expanded && (
          <>
                  ...
                  block.available && (
                    <View key={block.id} style={{ margin: 5, width: "45%" }}>
                      <DraggableBlock
                        ...
                        onDragEnd={(id, position, slotIndex, distanceMoved) =>
                          onDragEnd(id, position, slotIndex, distanceMoved)
                        }
                      />
                    </View>
            ...
            >
              ...
              >
                Drag elements belonging to the same category!
              </Text>
            </View>
          </>
        )}
...
export default function DragNDrop() {
  ...
  const [dropAreaLayout, setDropAreaLayout] = useState({
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  // Create fixed positions for slots with reduced vertical spacing
  const slotPositions = [
    { x: 20, y: 50 },
    { x: 20, y: 120 },
    { x: 20, y: 190 },
  ...
  // Minimum distance to consider a drag as intentional drop to a slot
  const DRAG_THRESHOLD = 80; // Increased threshold to make it harder to accidentally drop
  ...
  useEffect(() => {
    if (blocks.length > 0) {
      if (droppedItems.length === correctOrder.length) {
        ...
        const currentOrder = sortedItems.map((item) => parseInt(item.id));
        const isCorrect = currentOrder.every(
          (id, index) => id === parseInt(correctOrder[index])
        );
  ...
  const findNearestSlot = (position) => {
    ...
    const nearestAvailableSlot = distances.find((slot) => {
      return !droppedItems.some(
        (item) => item.slotIndex === slot.index && item.id !== activeDragId
      );
    });
  ...
  const swapItemsInSlots = (draggedId, targetSlotIndex) => {
    ...
    const itemInTargetSlot = droppedItems.find(
  ...
  const handleDragEnd = (
    ...
    // If the distance moved is very small and not from a slot, just keep it in the tray
    if (distanceMoved < DRAG_THRESHOLD && fromSlotIndex === null) {
      console.log(
        `Minimal movement detected (${distanceMoved}px), keeping in tray`
      );
    ...
    if (isInDropArea && distanceMoved >= DRAG_THRESHOLD) {
      ...
      if (nearestSlot !== -1) {
        ...
        if (fromSlotIndex !== null) {
          ...
          const itemInSlot = droppedItems.find(
            (item) => item.slotIndex === nearestSlot
          );
          ...
          if (itemInSlot) {
            ...
            setDroppedItems(
              droppedItems.filter((item) => item.slotIndex !== nearestSlot)
            );
          }
          ...
          if (block && block.available) {
            ...
            setBlocks(
              blocks.map((b) => (b.id === id ? { ...b, available: false } : b))
            );
      ...
      // If dragged out of the drop area and was from a slot, return to tray
      handleRemoveItem(id);
    } else {
      // If the block is from the tray and not dragged to the drop area,
      // do nothing - this ensures it stays in the tray
      console.log("Block remained in tray");
    }
  ...
  const handleRearrange = (id, position, fromSlotIndex, distanceMoved) => {
    // Use the same drag end handler with the slot index
    handleDragEnd(id, position, fromSlotIndex, distanceMoved);
  };
...
<DropArea
  ...
  setDroppedItems={setDroppedItems}  // Pass setDroppedItems
